...
import { getEquipment, deleteEquipmentById } from 'apiSdk/equipment';
...
function EquipmentListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('equipment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/equipment/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('equipment', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/equipment/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('equipment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/equipment/edit/${record.id}`} passHref legacyBehavior>
                          <Button
